...
3. If the task produces visible UI output (pages, components, layout, styling), start the dev server (`pnpm dev`) and use Playwright MCP tools to verify the rendered output. Use `mcp__playwright__browser_navigate` to go to the route, `mcp__playwright__browser_snapshot` for accessibility tree, and `mcp__playwright__browser_take_screenshot` to capture visuals. Use `mcp__playwright__browser_resize` for responsive checks. Confirm the implementation matches spec expectations. If visual issues are found, fix them before proceeding. If issues cannot be resolved in this iteration, document them in @IMPLEMENTATION_PLAN.md.
...
5. When all validations pass (build + typecheck + lint + Playwright verify if UI), update @IMPLEMENTATION_PLAN.md, then `git add -A` then `git commit` with a message describing the changes. After the commit, `git push`.
...
9999999999999999. Playwright verification for UI tasks: when the task involves pages, components, or styling, you MUST visually verify using Playwright MCP tools before committing. Start dev server, use `mcp__playwright__browser_navigate` to the route, `mcp__playwright__browser_take_screenshot` to capture, confirm layout/styling/content matches spec. Use `mcp__playwright__browser_resize` to check mobile (375x812) and desktop (1280x800). Document any visual deviations in @IMPLEMENTATION_PLAN.md.
99999999999999999. Auth: Claude Agent SDK uses Claude Code Max subscription — no API key needed. ElevenLabs uses signed URL auth flow (no browser credentials needed). Do not create or look for .env API keys for these services.
